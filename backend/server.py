...
async def upload_file(file: UploadFile = File(...)):
    ...
    sample = data.head().replace([np.inf, -np.inf, np.nan], None).to_dict(orient="records")
...
async def append_data(file: UploadFile = File(...)):
    ...
    sample = df.head().replace([np.inf, -np.inf, np.nan], None).to_dict(orient="records")
    sample = jsonable_encoder(sample)
...
async def train(req: Request):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=req.split)

    abc = AdaBoostClassifier(n_estimators=req.n_estimators, learning_rate=req.learning_rate, random_state=req.random_state)
    lr = LogisticRegression(max_iter=req.max_iter)
    rf = RandomForestClassifier()

    stack = StackingClassifier(
            estimators=[("lr", lr), ("rf", rf), ("abc", abc)],
            final_estimator=LogisticRegression(max_iter=req.max_iter_final, multi_class="multinomial"),
    ...
    cross = cross_validate(stack, X_train, y_train, cv=cv, scoring=scoring, n_jobs=-1)
    ...
    if os.path.exists("model.joblib"):
        ...
        new_score = (new_model_accuracy + new_model_recall + new_model_precision) / 3
